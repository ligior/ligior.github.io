...
document.addEventListener('DOMContentLoaded', function() {
	...
	document.addEventListener('mousemove', (e)=>{
		tooltipmain.style.left = event.clientX+"px";
		tooltipmain.style.top = event.clientY+"px";
	});
	var c = 0;
...
function gameend()
{
	...
	{
		zones[i]["object"].style.fill='rgba(255,255,255,0)';
		zones[i]["object"].onclick = function(event){ rollreset(i); };
		zones[i]["object"].onmouseenter = function(event) {
			tooltipmain.innerText = zones[i]["name"];
			zones[i]["object"].style.fill='rgba(255,255,224,0.5)';
		};
		zones[i]["object"].onmouseleave = function() {
			tooltipmain.innerText = "";
			zones[i]["object"].style.fill='rgba(0,0,0,0)';
		};
	}
}
function rollreset(ii)
{
	...
	for (let i = 0; i < zones.length; i++)
	{
		...
		{
			zones[i]["object"].style.fill='rgba(0,0,0,0.5)';
		}
		zones[i]["object"].onmouseenter = function(event) { event.target.style.fill = 'rgba(255,255,224,0.5)'; };
		zones[i]["object"].onmouseleave = function(event) { event.target.style.fill = 'rgba(0,0,0,0.5)'; };
	}
